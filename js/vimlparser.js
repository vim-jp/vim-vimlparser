...
VimLParser.prototype.check_missing_enddef = function(ends, pos) {
    if (this.context[0].type == NODE_DEF) {
        throw Err(viml_printf("E126: Missing :enddef:    %s", ends), pos);
    }
}
...
VimLParser.prototype.parse = function(reader) {
    ...
    this.check_missing_enddef("TOPLEVEL", this.reader.getpos());
...
VimLParser.prototype._parse_command = function(parser) {
    ...
    else if (parser == "parse_cmd_enddef") {
        this.parse_cmd_enddef();
    }
...
VimLParser.prototype.parse_cmd_function_or_def = function(is_def) {
    var pos = this.reader.tell();
    this.reader.skip_white();
    // :function
    ...
    var node = Node(is_def ? NODE_DEF : NODE_FUNCTION);
    node.pos = this.ea.cmdpos;
    node.body = [];
    node.ea = this.ea;
    node.left = left;
    node.rlist = [];
    node.default_args = [];
    node.attr = {"range":0, "abort":0, "dict":0, "closure":0, "return_type":NIL};
    ...
    if (tokenizer.peek().type == TOKEN_PCLOSE) {
        ...
        while (TRUE) {
            ...
            if (token.type == TOKEN_IDENTIFIER) {
                if (is_def) {
                    var token = this.parse_defarg(tokenizer, token);
                }
                else {
                ...
                if (token.type == TOKEN_COMMA) {
                    ...
                    if (is_def && !iswhite(p) && p != "<EOL>") {
                        throw Err("E1069: White space required after ','", token.pos);
                    }
                }
            ...
            else if (is_def && token.type == TOKEN_EOL) {
                if (tokenizer.peek().type == TOKEN_PCLOSE) {
                    tokenizer.get();
                    break;
                }
...
VimLParser.prototype.parse_defarg = function(tokenizer, current_token) {
    ...
    if (token.value.slice(-1) == ":") {
        // fix token.value. 'foo:' -> 'foo'
        token.value = token.value.slice(0, -2 + 1);
        tokenizer.reader.seek_cur(-1);
    }
    else {
        var i = viml_stridx(token.value, ":");
        if (i != -1) {
            // e.g. 'foo:number'
            token.pos.col += i + 1;
            throw Err("E1069: White space required after ':'", token.pos);
    ...
    else if (tokenizer.parse_type() === NIL) {
        throw Err(viml_printf("E1077: Missing argument type for %s", token.value), token.pos);
    }
    return token;
}

VimLParser.prototype.parse_cmd_function = function() {
    this.parse_cmd_function_or_def(FALSE);
}

VimLParser.prototype.parse_cmd_def = function() {
    this.parse_cmd_function_or_def(TRUE);
}

VimLParser.prototype.paser_cmd_endfunction_or_enddef = function(is_def) {
    var cmd = is_def ? "ENDDEF" : "ENDFUNCTION";
    this.check_missing_endif(cmd, this.ea.cmdpos);
    this.check_missing_endtry(cmd, this.ea.cmdpos);
    this.check_missing_endwhile(cmd, this.ea.cmdpos);
    this.check_missing_endfor(cmd, this.ea.cmdpos);
    if (is_def) {
        if (this.context[0].type != NODE_DEF) {
            throw Err("E193: :enddef not inside a def", this.ea.cmdpos);
        }
    }
    else {
        if (this.context[0].type != NODE_FUNCTION) {
    ...
    this.reader.getn(-1);
    var node = Node(is_def ? NODE_ENDDEF : NODE_ENDFUNCTION);
    node.pos = this.ea.cmdpos;
    node.ea = this.ea;
...
VimLParser.prototype.parse_cmd_endfunction = function() {
    this.paser_cmd_endfunction_or_enddef(FALSE);
}

VimLParser.prototype.parse_cmd_enddef = function() {
    this.paser_cmd_endfunction_or_enddef(TRUE);
}
...
VimLParser.prototype.parse_cmd_return = function() {
    var defindex = this.find_context(NODE_DEF);
    if (this.find_context(NODE_FUNCTION) == -1 && defindex == -1) {
    ...
    if (defindex != -1) {
        var return_type = this.context[defindex].attr.return_type;
        if (node.left !== NIL && return_type === NIL) {
            throw Err("E1096: Returning a value in a function without a return type", this.ea.cmdpos);
        }
        else if (node.left === NIL && return_type !== NIL) {
            throw Err("E1003: Missing return value", this.ea.cmdpos);
        }
    }
    this.add_node(node);
}
...
VimLParser.prototype.parse_cmd_let = function() {
    ...
    if (this.find_context(NODE_DEF) != -1) {
        throw Err("E1126: Cannot use :let in Vim9 script", this.ea.cmdpos);
    }
...
ExprTokenizer.prototype.parse_type = function() {
    var c = this.reader.p(0);
    if (c != ":") {
        return NIL;
    }
    ...
    if (!iswhite(this.reader.p(0))) {
        throw Err("E1069: White space required after ':'", this.reader.getpos());
    }
    this.reader.skip_white();
    var s = "";
    var generics = 0;
...
Compiler.prototype.compile = function(node) {
    if (node.type == NODE_TOPLEVEL) {
        ...
        this.compile_function_or_def(node, FALSE);
        return NIL;
    }
    else if (node.type == NODE_DEF) {
        this.compile_function_or_def(node, TRUE);
        return NIL;
    }
...
Compiler.prototype.compile_function_or_def = function(node, is_def) {
    ...
    this.out("(%s (%s)", is_def ? "def" : "function", left);
    this.incindent("  ");
    this.compile_body(node.body);
    this.out(")");
    this.decindent();
