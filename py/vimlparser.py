...
class VimLParser:
    ...
    def check_missing_enddef(self, ends, pos):
        if self.context[0].type == NODE_DEF:
            raise VimLParserException(Err(viml_printf("E126: Missing :enddef:    %s", ends), pos))
    ...
    def parse(self, reader):
        ...
        self.check_missing_enddef("TOPLEVEL", self.reader.getpos())
    ...
    def _parse_command(self, parser):
        ...
        elif parser == "parse_cmd_enddef":
            self.parse_cmd_enddef()
    ...
    def parse_cmd_function_or_def(self, is_def):
        pos = self.reader.tell()
        self.reader.skip_white()
        # :function
        ...
        node = Node(NODE_DEF if is_def else NODE_FUNCTION)
        node.pos = self.ea.cmdpos
        node.body = []
        node.ea = self.ea
        node.left = left
        node.rlist = []
        node.default_args = []
        node.attr = AttributeDict({"range": 0, "abort": 0, "dict": 0, "closure": 0, "return_type": NIL})
        ...
        if tokenizer.peek().type == TOKEN_PCLOSE:
            ...
            while TRUE:
                ...
                if token.type == TOKEN_IDENTIFIER:
                    if is_def:
                        token = self.parse_defarg(tokenizer, token)
                    else:
                    ...
                    if viml_has_key(named, token.value):
                    ...
                    if token.type == TOKEN_COMMA:
                        ...
                        if tokenizer.peek().type == TOKEN_PCLOSE:
                            tokenizer.get()
                            break
                        if is_def and not iswhite(p) and p != "<EOL>":
                            raise VimLParserException(Err("E1069: White space required after ','", token.pos))
                    ...
                    if token.type == TOKEN_PCLOSE:
                        ...
                        raise VimLParserException(Err(viml_printf("unexpected token: %s", token.value), token.pos))
                elif is_def and token.type == TOKEN_EOL:
                    if tokenizer.peek().type == TOKEN_PCLOSE:
                        tokenizer.get()
                        break
                else:
                    raise VimLParserException(Err(viml_printf("unexpected token: %s", token.value), token.pos))
        if is_def:
            node.attr.return_type = tokenizer.parse_type()
        else:
            while TRUE:
                self.reader.skip_white()
    ...
    def parse_defarg(self, tokenizer, current_token):
        ...
        if token.value[-1:] == ":":
            ...
            token.value = token.value[0:-2 + 1]
            tokenizer.reader.seek_cur(-1)
        else:
            i = viml_stridx(token.value, ":")
            if i != -1:
                # e.g. 'foo:number'
                token.pos.col += i + 1
                raise VimLParserException(Err("E1069: White space required after ':'", token.pos))
        if not isargname(token.value):
            raise VimLParserException(Err(viml_printf("E125: Illegal argument: %s", token.value), token.pos))
        elif tokenizer.parse_type() is NIL:
            raise VimLParserException(Err(viml_printf("E1077: Missing argument type for %s", token.value), token.pos))
        return token

    def parse_cmd_function(self):
        self.parse_cmd_function_or_def(FALSE)

    def parse_cmd_def(self):
        self.parse_cmd_function_or_def(TRUE)

    def paser_cmd_endfunction_or_enddef(self, is_def):
        cmd = "ENDDEF" if is_def else "ENDFUNCTION"
        self.check_missing_endif(cmd, self.ea.cmdpos)
        self.check_missing_endtry(cmd, self.ea.cmdpos)
        self.check_missing_endwhile(cmd, self.ea.cmdpos)
        self.check_missing_endfor(cmd, self.ea.cmdpos)
        if is_def:
            if self.context[0].type != NODE_DEF:
                raise VimLParserException(Err("E193: :enddef not inside a def", self.ea.cmdpos))
        else:
            if self.context[0].type != NODE_FUNCTION:
                raise VimLParserException(Err("E193: :endfunction not inside a function", self.ea.cmdpos))
        self.reader.getn(-1)
        node = Node(NODE_ENDDEF if is_def else NODE_ENDFUNCTION)
        node.pos = self.ea.cmdpos
        node.ea = self.ea
    ...
    def parse_cmd_endfunction(self):
        self.paser_cmd_endfunction_or_enddef(FALSE)

    def parse_cmd_enddef(self):
        self.paser_cmd_endfunction_or_enddef(TRUE)
    ...
    def parse_cmd_return(self):
        defindex = self.find_context(NODE_DEF)
        if self.find_context(NODE_FUNCTION) == -1 and defindex == -1:
        ...
        if defindex != -1:
            return_type = self.context[defindex].attr.return_type
            if node.left is not NIL and return_type is NIL:
                raise VimLParserException(Err("E1096: Returning a value in a function without a return type", self.ea.cmdpos))
            elif node.left is NIL and return_type is not NIL:
                raise VimLParserException(Err("E1003: Missing return value", self.ea.cmdpos))
        self.add_node(node)
    ...
    def parse_cmd_let(self):
        ...
        if self.ends_excmds(self.reader.peek()):
            ...
            self.parse_cmd_common()
            return
        if self.find_context(NODE_DEF) != -1:
            raise VimLParserException(Err("E1126: Cannot use :let in Vim9 script", self.ea.cmdpos))
...
class ExprTokenizer:
    ...
    def parse_type(self):
        ...
        if c != ":":
            return NIL
        self.reader.seek_cur(1)
        if not iswhite(self.reader.p(0)):
            raise VimLParserException(Err("E1069: White space required after ':'", self.reader.getpos()))
        self.reader.skip_white()
        s = ""
        generics = 0
        while TRUE:
            if generics != 0:
                ...
                if c == "<":
                    ...
                    raise VimLParserException(Err("unexpected EOL", self.reader.getpos()))
                elif not iswordc(c) and c != "." and c != ",":
...
class Compiler:
    ...
    def compile(self, node):
        if node.type == NODE_TOPLEVEL:
            ...
            self.compile_function_or_def(node, FALSE)
            return NIL
        elif node.type == NODE_DEF:
            self.compile_function_or_def(node, TRUE)
            return NIL
    ...
    def compile_function_or_def(self, node, is_def):
        ...
        if not viml_empty(rlist):
            ...
            if remaining:
                left += " . ..."
        self.out("(%s (%s)", "def" if is_def else "function", left)
        self.incindent("  ")
        self.compile_body(node.body)
        self.out(")")
        self.decindent()
